...
export default function GlobalNotificationProvider() {
  ...
  useEffect(() => {
    ...
    console.log("[GlobalNotification] Initializing Realtime subscription...");
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        ...
        if (status === 'SUBSCRIBED') {
          console.log("[GlobalNotification] Connection healthy and listening.");
        } else if (status === 'CHANNEL_ERROR') {
          console.error("[GlobalNotification] ERROR: Realtime connection failed. Check env vars or Supabase project status.");
        }
      });
  ...
  const checkReminders = useCallback(async () => {
    try {
        ...
        console.warn("[GlobalReminder] Background check issue:", error?.message);
    }
  }, [showedSummaryDate]);
  ...
  const confirmApproval = async () => {
    ...
    try {
      ...
      alert("Success: Appointment approved and email sent.");
    } catch (err) {
